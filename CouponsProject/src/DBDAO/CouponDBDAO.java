...
public class CouponDBDAO implements CouponDAO<Coupon> {
	...
	public boolean doesCouponExist(String title, int companyID) throws Exception {
		...
		try {
			...
			String sql = String.format(
					"SELECT COUNT(*) AS count FROM Coupons WHERE title = '%s' AND company_id = '%s' ", title,
					companyID);
	...
	public void updateCoupon(Coupon coupon) throws Exception {
		...
		try {
			...
			String sql = String.format(
					...
					coupon.getCategory_id(), coupon.getTitle(), coupon.getDescription(), startDate, endDate,
					coupon.getAmount(), coupon.getPrice(), coupon.getImage(), coupon.getId());
		...
		if (!isCouponAvailble(couponID) || isCouponPurchased(customerID, couponID)) {
			...
			try {
				...
				String sql = String.format("INSERT INTO Customers_vs_Coupons (customer_id, coupon_id) VALUES(%d, %d)",
						customerID, couponID);
				String amountSet = String.format("UPDATE Coupons SET amount = (amount-1) WHERE coupon_id = %d ",
						couponID);
				try (PreparedStatement preparedStatement = connection.prepareStatement(sql,
						PreparedStatement.RETURN_GENERATED_KEYS)) {
					// execute
	...
	public ArrayList<Coupon> getCouponsByPrice(int companyID, double couponPrice) throws Exception {
		...
		try {
			...
			// prepare query
			String sql = String.format("SELECT FROM Coupons WHERE company_id = %d AND price = %.2f", companyID,
					couponPrice);
			try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
				// execute query
				try (ResultSet resultSet = preparedStatement.executeQuery()) {
						...
						try (PreparedStatement preparedStatement2 = connection.prepareStatement(sql2)) {
							try (ResultSet resultSet2 = preparedStatement2.executeQuery()) {
								...
								while (resultSet2.next()) {
									...
									Coupon coupon = new Coupon(coupon_id, company_id, category_id, title, description,
											image, start_date, end_date, amount, price);
		...
		try {
			...
			try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
				try (ResultSet resultSet = preparedStatement.executeQuery()) {
					...
					while (resultSet.next()) {
						int coupon_id = resultSet.getInt("coupon_id");
						String sql2 = String.format("SELECT * FROM Coupons WHERE coupon_id = %d AND category_id = %d",
								coupon_id, categoryID);
						...
						try (PreparedStatement preparedStatement2 = connection.prepareStatement(sql2)) {
							try (ResultSet resultSet2 = preparedStatement2.executeQuery()) {
								...
								while (resultSet2.next()) {
									...
									Coupon coupon = new Coupon(coupon_id, company_id, categoryID, title, description,
											image, start_date, end_date, amount, price);
									customerCouponList.add(coupon);
								}
		...
		try {
			...
			try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
				try (ResultSet resultSet = preparedStatement.executeQuery()) {
					...
					while (resultSet.next()) {
						int coupon_id = resultSet.getInt("coupon_id");
						String sql2 = String.format("SELECT * FROM Coupons WHERE coupon_id = %d AND price = %.2f",
								coupon_id, couponPrice);
						...
						try (PreparedStatement preparedStatement2 = connection.prepareStatement(sql2)) {
							try (ResultSet resultSet2 = preparedStatement2.executeQuery()) {
								...
								while (resultSet2.next()) {
									...
									Coupon coupon = new Coupon(coupon_id, company_id, category_id, title, description,
											image, start_date, end_date, amount, couponPrice);
									customerCouponList.add(coupon);
								}
